#!/usr/bin/env python3
"""
Slack → Expensify Reimbursement Bot (v3)
=======================================

Changes in **v3** ("richer status")
-----------------------------------
* Poller now inspects **`transactionStatus`** returned by Expensify:
    * `PROCESSING`, `VALIDATING`, etc. → bot posts the exact state.
    * `COMPLETED` → SmartScan done, bot shows merchant/amount/date.
    * `ERROR`  → bot warns with the error text (if any).
* Amount‑is‑zero fallback kept for accounts that don’t expose `transactionStatus`.
* Config knobs (`POLL_INTERVAL_SEC`, `MAX_POLLS`) unchanged.

Everything else (setup, scopes, deployment) stays the same.
"""
    ...
    for attempt in range(1, MAX_POLLS + 1):
        ...
        if not exp:
            ...
            app.client.chat_postMessage(
                channel=channel,
                thread_ts=thread_ts,
                text=f"⌛ SmartScan status = *NOT_YET_SYNCED* (attempt {attempt}/{MAX_POLLS})",
            )
            continue
        ...
        if status and status.upper() not in {"COMPLETED", "ERROR"}:
            app.client.chat_postMessage(
                channel=channel,
                thread_ts=thread_ts,
                text=f"⌛ SmartScan status = *{status}* (attempt {attempt}/{MAX_POLLS})",
            )
            continue
        ...
        # status == COMPLETED (or amount populated)
    ...
    for file_info in files:
        ...
        if resp.status_code != 200:
            say(
                ...
                text=f"⚠️ Could not download *{file
